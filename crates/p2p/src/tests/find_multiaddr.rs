...
// Test in which we test command [`Command::FindMultiaddr`].
#[tokio::test(flavor = "multi_thread", worker_threads = 10)]
async fn test_find_multiaddr_extremelly_long() -> anyhow::Result<()> {
    init_tracing();
    const USERS_NUM: usize = 9;
    ...
    let Setup {
        ...
        cancel,
        tasks,
    } = Setup::all_to_all_with_custom_timer(
        USERS_NUM,
    ...
    info!("Waiting insane amount of time because we set insanely small timers for put record");
