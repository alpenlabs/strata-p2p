...
async fn test_reqresp_mute_then_unmute() -> anyhow::Result<()> {
    ...
    // Drain the event generated by the "mute reqresp" request
    let _ = timeout(Duration::from_secs(2), user1.reqresp.next_event()).await;
    ...
    sleep(Duration::from_millis(800)).await; // give unmute a moment
    user0
        .reqresp
        .send(RequestResponseCommand {
            target_transport_id: user1.peer_id,
    ...
    let after_unmute = timeout(Duration::from_secs(3), user1.reqresp.next_event()).await;
    assert!(
        after_unmute.is_ok(),
